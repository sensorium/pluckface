...
aubio_pitchfcomb_do (aubio_pitchfcomb_t * p, const fvec_t * input, fvec_t * output)
{
  ...
  for (l = 1; l < MAX_PEAKS && peaks[l].bin > 0.0; l++) {
    sint_t onset_ptronic;
    for (onset_ptronic = 5; onset_ptronic > 1; onset_ptronic--) {
      if (peaks[0].bin / peaks[l].bin < onset_ptronic + .02 &&
          peaks[0].bin / peaks[l].bin > onset_ptronic - .02) {
        if (onset_ptronic > (sint_t) maxonset_ptr && peaks[0].db < peaks[l].db / 2) {
          maxonset_ptr = onset_ptronic;
          k = l;
        }
      }
    }
